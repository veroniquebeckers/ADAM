...
// public int getSurvivalSize ()
	...
	public int getNextCoverType(int year, float area, Parcel parcel) {
		...
		if (currentCover != Config.farm_house && currentCover != Config.agr_buildings) {
			// Loop over all crop types to calculate their productivity
			double[] probs = new double[cropMap.size() + 1];
			...
			if (currentCover == 107) {
				for (Integer crop : cropMap.keySet()) {
					double gain = parcel.getProductivityForCrop(year, crop)
							* getRotationChanceForCrop(cropMap.get(currentCover), crop);
					// float gain = Market.getMarketValue(year, crop) * area
					// *parcel.getProductivityForCrop(year, crop) ;
					// float gain = area *parcel.getProductivityForCrop(year,
					// crop);
					crops.add(crop);
					totalProb += gain;
					probs[c] = gain;
				...
				for (Integer crop : cropMap.keySet()) {
					double gain = Market.getMarketValue(year, crop) * area * parcel.getProductivityForCrop(year, crop)
							* getRotationChanceForCrop(cropMap.get(currentCover), crop);
					// float gain = Market.getMarketValue(year, crop) * area
					// *parcel.getProductivityForCrop(year, crop) ;
					// float gain = area *parcel.getProductivityForCrop(year,
			...
			if (totalProb == 0) {
				throw new Error("No available crops for farmer to choose. All productivities are 0 for parcel "
						+ parcel.getID() + " in year " + year);
			}
			

			// Values in "probs" are absolute, change them to relative
			// cumulative
			for (int i = 1; i < probs.length; i++) {
				// System.out.println(probs[i] + " ---- " +totalProb);
				double t = probs[i] / totalProb;
				probs[i] /= totalProb;
				probs[i] += probs[i - 1];
